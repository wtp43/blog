...
export default async function HomePage() {
  ...
  return (
    <>
      <div className="grid gap-24 lg:gap-32 top-0 mt-6 lg:mt-12 md:grid-cols-[.618fr,1fr] h-full">
        <main className="space-y-8 lg:min-h-screen">
          <AboutMe />
          <ProjectList
            showYears={false}
        ...
        <span className="flex flex-col gap-2">
          ...
          {process.env.NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA ? (
            <span>
              <Link
                external
                href={`https://github.com/wtp43/
blog/commit/${process.env.NEXT_PUBLIC_VERCEL_GIT_COMMIT_SHA}`}
              >
